...
work_dir = "/opt/ml/detection/baseline/mmdetection/work_dirs/cascade"
...
# data settings
data = dict(
    samples_per_gpu=4, # batch size for GPU
    workers_per_gpu=2, # num_workers
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'stratified_kfold/cv_train_1.json',  # use entire train dataset
...
# evaluation = dict(interval = 3, metric="bbox", save_best="val/bbox_mAP_50")
name = "MMdetection"
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(type="WandbLoggerHook", interval=500, init_kwargs=dict(project="test-project", entity="winner", name= name))
    ])
# yapf:enable
