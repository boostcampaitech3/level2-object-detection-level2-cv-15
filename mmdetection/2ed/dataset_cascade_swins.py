...
albu_train_transforms = [
    ...
    p=0.5),
    dict(type='RandomResizedCrop',height=768, width=768, scale=(0.5, 1.0), p=0.5),
...
# data settings
data = dict(
    samples_per_gpu=14, # batch size for GPU
    workers_per_gpu=3, # num_workers
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'cv_train_5.json',  # use entire train dataset
        img_prefix=data_root,
        classes=classes,
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'val_5.json',
        img_prefix=data_root,
        classes=classes,
